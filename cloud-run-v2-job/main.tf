...
resource "google_cloud_scheduler_job" "job" {
  ...
  dynamic "retry_config" {
    for_each = var.scheduler.retry_config == null ? [] : [""]
    content {
      min_backoff_duration = var.scheduler.min_backoff_duration
      max_retry_duration = var.scheduler.max_retry_duration
      max_doublings = var.scheduler.max_doublings
      retry_count = var.scheduler.retry_count
    }
  }

  dynamic "pubsub_target" {
    for_each = var.scheduler.pubsub_target == null ? [] : [""]
    content {
      topic_name = var.scheduler.topic_name
      data = var.scheduler.data
      attributes = var.scheduler.attributes
    }
  }

  dynamic "app_engine_http_target" {
    for_each = var.scheduler.app_engine_http_target == null ? [] : [""]
    content {
      http_method = var.scheduler.http_method
      dynamic "app_engine_routing" {
        for_each = var.scheduler.app_engine_routing == null ? [] : [""]
        iterator = app_engine_routing
        content {
          service = app_engine_routing.service
          version = app_engine_routing.version
          instance = app_engine_routing.instance
        }
      }
      relative_uri = var.scheduler.relative_uri
      body = var.scheduler.body
      headers = var.scheduler.headers
    }
  }

  dynamic "http_target" {
    for_each = var.scheduler.http_target == null ? [] : [""]
    content {
      uri = var.scheduler.uri
      http_method = var.scheduler.http_method
      body = var.scheduler.body
      headers = var.scheduler.headers
      dynamic "outh_token" {
        for_each = var.scheduler.http_target.outh_token == null ? [] : [""]
        iterator = outh_token
        content {
          service_account_email = outh_token.service_account_email
          scope = outh_token.scope
        }
      }

      dynamic "oidc_token" {
        for_each = var.scheduler.http_target.oidc_token == null ? [] : [""]
        iterator = oidc_token
        content {
          service_account_email = oidc_token.service_account_email
          audience = oidc_token.audience
        }
      }
    }
  }
